	...
	rmt_channel_handle_t *ret_chan)
{
	...
	if (config->flags.with_dma) {
		...
		bool priority_conflict = rmt_set_intr_priority_to_group(group,
	...
	esp_rom_gpio_connect_in_signal(config->gpio_num,
		rmt_periph_signals.groups[group_id].channels[channel_id
			+ RMT_RX_CHANNEL_OFFSET_IN_GROUP].rx_sig, config->flags.invert_in);
	...
	*ret_chan = &rx_channel->base;
	LOG_DBG("new rx channel(%d,%d) at %p, gpio=%d, res=%"PRIu32"Hz, hw_mem_base=%p, \
		ping_pong_size=%d", group_id, channel_id, rx_channel, config->gpio_num,
		rx_channel->base.resolution_hz, rx_channel->base.hw_mem_base,
		rx_channel->ping_pong_symbols);
	return 0;
	...
	const rmt_carrier_config_t *config)
{
	...
	if (config && config->frequency_hz) {
		...
		key = k_spin_lock(&channel->spinlock);
		rmt_ll_rx_set_carrier_level(hal->regs, channel_id,
	...
	if (real_frequency > 0) {
		LOG_DBG("enable carrier demodulation for channel(%d,%d), freq=%"PRIu32"Hz", group_id,
			channel_id, real_frequency);
	} else {
