 ...
 * @retval -ENOMEM if creating RMT TX channel failed because out of memory.
 * @retval -ENOMEM if creating RMT TX channel failed because all RMT channels are used up and no more free one.
 * @retval -ENODEV if creating RMT TX channel failed because some feature is not supported by hardware, e.g. DMA feature is not supported by hardware.
 * @retval -ENODEV if creating RMT TX channel failed because of other error.
 */
int rmt_new_tx_channel(const struct device *dev, const rmt_tx_channel_config_t *config, rmt_channel_handle_t *ret_chan);
 ...
 * @retval -ENODEV Transmit data failed because channel is not enabled.
 * @retval -ENODEV Transmit data failed because some feature is not supported by hardware, e.g. unsupported loop count.
 * @retval -ENODEV Transmit data failed because of other error.
 */
int rmt_transmit(rmt_channel_handle_t tx_channel, rmt_encoder_handle_t encoder, const void *payload, size_t payload_bytes, const rmt_transmit_config_t *config);
 ...
 */
int rmt_tx_register_event_callbacks(rmt_channel_handle_t tx_channel, const rmt_tx_event_callbacks_t *cbs, void *user_data);
 ...
 * @retval -ENODEV Create sync manager failed because it is not supported by hardware.
 * @retval -ENODEV Create sync manager failed because not all channels are enabled.
 * @retval -ENOMEM Create sync manager failed because out of memory.
 * @retval -ENOMEM Create sync manager failed because all sync controllers are used up and no more free one.
 * @retval -ENODEV Create sync manager failed because of other error.
 */
