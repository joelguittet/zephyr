...
static int rmt_rx_init_dma_link(const struct device *dev, rmt_rx_channel_t *rx_channel, const rmt_rx_channel_config_t *config)
{
    ...
    if (rc) {
        LOG_ERR("Failed to configure DMA channel: %" PRIu32" (%d)", cfg->rx_dma_channel, rc);
        return rc;
    }
...
static int rmt_rx_destroy(rmt_rx_channel_t *rx_channel)
{
    ...
    if (rx_channel->base.intr) {
        // FIXME: freeing the intr makes the system a FATAL EXCEPTION
...
int rmt_new_rx_channel(const struct device *dev, const rmt_rx_channel_config_t *config, rmt_channel_handle_t *ret_chan)
{
    int rc;
    esp_err_t ret;
    k_spinlock_key_t key;
...
#if SOC_RMT_SUPPORT_DMA
    if (channel->dma_dev) {
        rc = dma_reload(channel->dma_dev, channel->dma_channel, 0, (uint32_t)channel->dma_mem_base, channel->dma_mem_size);
        if (rc) {
    ...
    int status)
{
    ...
    if (rx_chan->on_recv_done) {
        ...
        desc = (dma_descriptor_t *)gdma_ll_rx_get_success_eof_desc_addr(dma_hal->dev, dma_channel / 2);
        if (!desc) {
