...
int rmt_select_periph_clock(rmt_channel_handle_t chan, rmt_clock_source_t clk_src)
{
    ...
    */
    ...
    esp_pm_lock_type_t pm_lock_type = chan->dma_dev ? ESP_PM_NO_LIGHT_SLEEP : ESP_PM_CPU_FREQ_MAX;
#else
...
#endif
    snprintf(chan->pm_lock_name, RMT_PM_LOCK_NAME_LEN_MAX, "rmt_%d_%d", group->group_id, channel_id); /* e.g. rmt_0_0 */
