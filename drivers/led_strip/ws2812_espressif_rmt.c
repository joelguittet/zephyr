...
struct ws2812_espressif_rmt_cfg {
...
{
	const struct ws2812_espressif_rmt_cfg *cfg = dev->config;
...
static int ws2812_espressif_rmt_init(const struct device *dev)
{
	...
	if (rc < 0) {
		LOG_ERR("Error configuring RMT pin (%d)", rc);
		return rc;
	}
	data->tx_chan_config.gpio_num = ESP32_PIN_NUM(cfg->pcfg->states[0].pins[0].pinmux);
	if (ESP32_PIN_MODE_OUT(cfg->pcfg->states[0].pins[0].pincfg) == ESP32_PIN_OUT_LOW) {
		data->tx_chan_config.flags.invert_out = 1;
	}
	if (ESP32_PIN_DRV(cfg->pcfg->states[0].pins[0].pincfg) == ESP32_OPEN_DRAIN) {
		data->tx_chan_config.flags.io_od_mode = 1;
	}
...
#define WS2812_ESPRESSIF_RMT_DEVICE(idx)                                                 \
	...
	static const struct ws2812_espressif_rmt_cfg ws2812_espressif_rmt_##idx##_cfg = {    \
	...
	static struct ws2812_espressif_rmt_data ws2812_espressif_rmt_##idx_data = {          \
		...
		.tx_chan_config.flags.with_dma = DT_INST_PROP(idx, with_dma),                    \
