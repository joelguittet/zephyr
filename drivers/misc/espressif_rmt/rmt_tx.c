...
static int rmt_tx_init_dma_link(const struct device *dev, rmt_tx_channel_t *tx_channel, const rmt_tx_channel_config_t *config)
{
	...
	/* Allocate memory */
	rmt_symbol_word_t *dma_mem_base = k_aligned_alloc(sizeof(uint32_t), sizeof(rmt_symbol_word_t) * config->mem_block_symbols);
	if (!dma_mem_base) {
	...
	if (rc) {
		LOG_ERR("Failed to configure DMA channel: %" PRIu32" (%d)", cfg->tx_dma_channel, rc);
		return rc;
	}
...
static int rmt_tx_create_trans_queue(rmt_tx_channel_t *tx_channel, const rmt_tx_channel_config_t *config)
{
	...
	for (int i = 0; i < RMT_TX_QUEUE_MAX; i++) {
		tx_channel->trans_queue_structs[i] = k_malloc(sizeof(rmt_tx_trans_desc_t *) * config->trans_queue_depth);
...
int rmt_new_tx_channel(const struct device *dev, const rmt_tx_channel_config_t *config, rmt_channel_handle_t *ret_chan)
{
	...
	ret = esp_intr_alloc_intrstatus(rmt_periph_signals.groups[group_id].irq, isr_flags,
		(uint32_t)rmt_ll_get_interrupt_status_reg(hal->regs), RMT_LL_EVENT_TX_MASK(channel_id),
		rmt_tx_default_isr, tx_channel, &tx_channel->base.intr);
	if (ret) {
...
#if SOC_RMT_SUPPORT_DMA
	if (channel->dma_dev) {
		rc = dma_reload(channel->dma_dev, channel->dma_channel, (uint32_t)channel->dma_mem_base, 0, channel->dma_mem_size);
		if (rc) {
